...
// Calcular enabled com logging para debug
$debugbarEnabled = env('DEBUGBAR_ENABLED');
$appDebug = env('APP_DEBUG', false);
$enabled = $debugbarEnabled ?? $appDebug;

// Logar quando enabled é null ou quando está sendo avaliado
$logMessage = sprintf(
    '[DEBUGBAR CONFIG] enabled=%s, DEBUGBAR_ENABLED=%s, APP_DEBUG=%s, timestamp=%s',
    $enabled ? 'true' : 'false',
    $debugbarEnabled === null ? 'null' : ($debugbarEnabled ? 'true' : 'false'),
    $appDebug ? 'true' : 'false',
    date('Y-m-d H:i:s')
);

// Logar em arquivo específico e error_log
$logFile = __DIR__ . '/../storage/logs/debugbar-config.log';
if (is_dir(dirname($logFile))) {
    @file_put_contents($logFile, $logMessage . PHP_EOL, FILE_APPEND | LOCK_EX);
}
// Também logar no error_log do PHP (vai para stderr ou php-fpm.log)
